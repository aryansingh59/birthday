...
// 1. AudioPlayer with auto-play interaction logic and romantic theme
...
content = content.replace(/const AudioPlayer = \(\) => \{[\s\S]*?return \([\s\S]*?<\/div>\s*\);\s*\};/, newAudioPlayer.trim());


// 2. VoiceFromHeart with robust playback logic and OpenAI audio link
...
content = content.replace(/const VoiceFromHeart = \(\) => \{[\s\S]*?return \([\s\S]*?<\/section>\s*\)\s*\}/, newVoiceFromHeart.trim());
